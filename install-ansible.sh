...
if [ $? -eq 1 ]; then
  ...
  if [ -z $ANSIBLE_DEBUG ]; then
    ...
    apt-get -qq --force-yes install git python-setuptools python-dev python-paramiko python-cryptography > /dev/null 2>&1
  else
    apt-get --force-yes update
    apt-get --force-yes install git python-setuptools python-dev python-paramiko python-cryptography
  fi
